...
4. [`seed_gabinetes.sql`](seed_gabinetes.sql) — 20ª Legislatura + 29 vereadores e seus gabinetes.
5. [`seed_comissoes.sql`](seed_comissoes.sql) — 30 comissões permanentes (biênio 2025/2026).
6. [`rls.sql`](rls.sql) — habilita RLS em todas as tabelas (padrão seguro).
...
psql "$DATABASE_URL" -f db/seed_comissoes.sql
...
- Estado atual: schema aplicado (migrations `mvp_schema_fase0_fase1`, `enable_rls_all_tables`,
  `parlamentar_gabinete_e_nome`) + seeds carregados — **76 órgãos** (47 da estrutura + 29 gabinetes),
  20ª Legislatura, **29 vereadores** (linkados aos gabinetes), **30 comissões permanentes**,
  8 tipos, 6 papéis, 8 permissões, 12 fases, 14 transições.
